...
func (c *ApplyViewer) Build(vgin *vugu.BuildIn) (vgout *vugu.BuildOut) {
	...
	vgout.Out = append(vgout.Out, vgn)	// root for output
	{
		...
		vgparent.AppendChild(vgn)
		{
			...
			vgparent.AppendChild(vgn)
			vgn.Attr = append(vgn.Attr, vugu.VGAttribute{Key: "aria-controls", Val: fmt.Sprint(fmt.Sprintf("%p-%p", c, c.Apply))})
			vgn.Attr = append(vgn.Attr, vugu.VGAttribute{Key: "data-target", Val: fmt.Sprint(fmt.Sprintf("#%p-%p", c, c.Apply))})
			{
		...
		vgparent.AppendChild(vgn)
		vgn.Attr = append(vgn.Attr, vugu.VGAttribute{Key: "id", Val: fmt.Sprint(fmt.Sprintf("%p-%p", c, c.Apply))})
		{
			vgparent := vgn
			_ = vgparent
			vgn = &vugu.VGNode{Type: vugu.VGNodeType(1), Data: "\n     "}
						...
						vgparent.AppendChild(vgn)
						{
							...
							for i, hashRequest := range c.Actions.Hash {
								...
								vgparent.AppendChild(vgn)
								{
									...
									vgparent.AppendChild(vgn)
									{
										vgcompKey := vugu.MakeCompKey(0x5F3C96710BAA5FCE, vgiterkey)
